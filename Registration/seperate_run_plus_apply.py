...
bfe_path = 'samples/data/BFE'
fake_ct_path = 'samples/data/fake_CT_30'
...
mask_fake_save_dir = f'mask_fake_{method}_cycle_ada'
if not os.path.isdir(ct_tmp_path):
    os.makedirs(ct_tmp_path)
if not os.path.isdir(mask_save_dir):
    os.makedirs(mask_save_dir)
if not os.path.isdir(mask_fake_save_dir):
    os.makedirs(mask_fake_save_dir)
...
def get_cycle_ada_outline(mask_init, input_array=True):
    ...
    for current_z in range(z_shape):
        ...
        y_array, x_array = yx_slice.nonzero()
        points_set = np.concatenate((y_array[:, np.newaxis], x_array[:, np.newaxis]), axis=1)  #[:, np.newaxis, :]
        # cnts, _ = cv.findContours(cv.threshold(yx_slice.astype(np.uint8), 0, 1, cv.THRESH_BINARY)[1], cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
        # (y_center,x_center),radius = cv.minEnclosingCircle(np.concatenate(cnts, axis=0)) # be careful that this is a (y,x) image
        (y_center, x_center), radius = cv.minEnclosingCircle(points_set)  # be careful that this is a (y,x) image
        y = np.arange(y_shape)
        x = np.arange(x_shape)
        Y, X = np.meshgrid(y,x)
        needed_slice = (((Y-y_center)**2 + (X-x_center)**2) < radius**2).astype(np.int16)
        # print(f"Y shape:{Y.shape}, needed_slice shape:{needed_slice.shape}, mask_init shape:{mask_init.shape}")
        mask_cycle_ada[current_z, :, :] = needed_slice.transpose((1, 0)) # transpose because meshgrid order is (横坐标先，纵坐标后)，需要调换一下
...
def registration_and_apply(fixed_image_path, moving_image_path, to_apply_image_path, method, save_dir, save_all=True):
    ...
    if save_all:
        ants.image_write(fake_img, os.path.join(save_dir, 'fake_ct_warpedmovout.nii.gz'))
...
def main():
    assert os.listdir(ct_path) == os.listdir(bfe_path) == os.listdir(fake_ct_path)
    ...
    for file in tqdm(file_list):
        try:
            ...
            fake_ct_img_path = os.path.join(fake_ct_path, file)
            fake_ct = sitk.ReadImage(fake_ct_img_path)
            fake_ct_array = sitk.GetArrayFromImage(fake_ct)
            bfe_img_path = os.path.join(bfe_path, file)
            ...
            mask_total = open_process(mask_total, size=2)
            mask_top, mask_bottom = segment(mask_total, os.path.join(mask_save_dir,str(index)))

            mask_fake_total = copy_itk_img_info(binary(fake_ct_array, bone_thres_fake), fake_ct, input_array=True)
            mask_fake_total = open_process(mask_fake_total, size=3)
            mask_fake_top, mask_fake_bottom = segment(mask_fake_total, os.path.join(mask_fake_save_dir, str(index)), use_y_division=True)
            if not isinstance(mask_top, np.ndarray) or not isinstance(mask_bottom, np.ndarray):
                logger.warning(f"error occurred in segmentation of two bones for: \n {file}!!!")
                error_file_dict[file] = 'error:0'
                # break #debug only
                continue
            if not isinstance(mask_fake_top, np.ndarray) or not isinstance(mask_fake_bottom, np.ndarray):
                logger.warning(f"error occurred in segmentation of two bones for: \n {file}!!!")
                error_file_dict[file] = 'error:1'
                # break #debug only
                continue
            ...
            img_fake_top = copy_itk_img_info(apply_mask(fake_ct_array, mask_fake_top, bg_value=bg_value), fake_ct, input_array=True)
            img_fake_top_save_path = os.path.join(fake_ct_tmp_path, f'{index}_top.nii')
            sitk.WriteImage(img_fake_top, img_fake_top_save_path)
            ...
            bfe_top = registration_and_apply(img_top_save_path, img_fake_top_save_path, bfe_img_path, method, os.path.join(save_path, f'{index}/top'), save_all=True)
            ...
            bfe_bottom = registration_and_apply(img_bottom_save_path, img_fake_bottom_save_path, bfe_img_path, method, os.path.join(save_path, f'{index}/bottom'), save_all=True)
